...
import { Menu, X, ArrowUp, Info, XCircle } from "lucide-react";
...
const SpinsTableComponent: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans">
      ...
      <div className="flex justify-between items-center p-4 border-b">
        ...
        <div>
          <input
            ...
            className="px-4 py-3 border rounded-lg text-sm max-w-xs outline-none focus:border-blue-500 cursor-pointer"
            readOnly
          />
        </div>
      ...
      {/* Menu Grid */}
      {menuOpen && (
        <div className="absolute right-8 top-20 bg-white border rounded-lg shadow-lg p-6 z-50">
          ...
          {/* Ad Section */}
          <div className="flex items-center border rounded-lg p-3 bg-white mt-4">
            <img
              src="https://res.cloudinary.com/dyueswnzk/image/upload/v1758701617/r3o4deralgc2jl1y1xag_ynrxbj.webp"
              alt="Ad"
              className="w-28 h-26 rounded mr-4"
            />
          ...
          <tbody>
            {tableData.map((row: ChartData, idx: number) => (
              ...
              >
                ...
                >
                  <div className="font-bold">{row.song}</div>
                  <div className="text-gray-500 text-xs mt-1">{row.artist}</div>
                </td>
                <td
                  className={`px-6 py-6 text-center text-sm transition-all ${
                    hoveredColumn === 2
                ...
                <td
                  className={`px-6 py-6 text-center text-sm transition-all ${
                    hoveredColumn === 10
                      ? "bg-orange-50 border-l border-r border-orange-600"
                      : ""
                  } ${idx === 0 && hoveredColumn === 10 ? "border-t-0" : ""} ${idx === tableData.length - 1 && hoveredColumn === 10 ? "border-b-2 border-orange-600" : ""}`}
                >
                  <a href="#" className="hover:underline">
                    {row.dj || "—"}
                  </a>
